...
def multi_gpu_launcher(args):
    ...
    if num_machines > 1:
        ...
        process_ids = np.arange(num_processes)
        machines_processes = np.array_split(process_ids, num_machines)
        setattr(args, "nproc_per_node", str(len(machines_processes[int(args.machine_rank)])))
