...
class Model(base.Model):
    ...
    def forward(self, vocab, **inputs):
        ...
        # embed language
        output = {}
        # collect the index of the <<question>> token of the batch
        index = (inputs['questions'] == 1).nonzero(as_tuple=False)[:, 1].cpu().detach().numpy()
        """print(index[0])"""
        ...
        decoder_input_alt = encoder_out_visual[:, -1, :]
        # print(decoder_input.shape)

        """# use outputs corresponding to language for prediction only
        encoder_out_language = encoder_out[
            :, :lengths_lang.max().item()]
        # print(encoder_out_language.shape)
        encoder_out_language_reshape = encoder_out_language.reshape(-1, self.args.demb)
        encoder_out_language_emb = self.emb_enc_lang(encoder_out_language_reshape)
        # print(encoder_out_language_emb.shape)
        encoder_out_language_emb = encoder_out_language_emb.view(*encoder_out_language.shape[:2], -1)
        # print(encoder_out_language_emb.shape)
        # selection the classes values for the <<question>> token
        ids = torch.cat([torch.ones(1, lengths_lang.max().item(), self.num_classes)*idx for idx in index]).long().cuda()
        encoder_out_language_emb_question = torch.gather(encoder_out_language_emb, 1, ids)[:, 0, :]"""

        """print(encoder_out_language_emb[0, 2, :])
        print(encoder_out_language_emb_question[0])"""
        # get the output objects
        # emb_object_flat = emb_object.view(-1, self.args.demb)
        emb_object_flat_alt = emb_object[:, -1, :]
        decoder_input = decoder_input_alt + emb_object_flat_alt
        # decoder_input = decoder_input + emb_object_flat
        # print(decoder_input.shape)
        answer_flat = self.dec_QA(decoder_input)
        # answers = answer_flat.view(*encoder_out_visual.shape[:2], *answer_flat.shape[1:])

        # print(answer_flat.shape)
        # print(answers.shape)
        ...
        output.update({'answers': answer_flat})
