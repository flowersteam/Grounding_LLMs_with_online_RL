...
class EncoderLang_QA(EncoderLang):
    def __init__(self, num_layers, args,
                 subgoal_token='<<instr>>', goal_token='<<goal>>', question_token='<<question>>'):
        '''
        transformer encoder for language inputs
        '''
        super(EncoderLang_QA, self).__init__(num_layers, args)
